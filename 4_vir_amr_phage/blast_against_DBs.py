...
def blast_all_clusters(input_dirs, dbs, args):
    ...
    for d in input_dirs:
        for gene_class in ["rare", "inter", "soft_core", "core"]:
            for db in dbs:
                print("Blast CLuster %s, Gene Type: %s, Against: %s" %
                      (d, gene_class, db))
                query_file = os.path.join(d, gene_class + "_genes.fa")
                out_file = os.path.join(
                    d, gene_class + "_" + db + "_blast_result.tab")
        ...
        with open(os.path.join(d, "summary_statistics.txt")) as f:
            for l in f:
                if l.startswith("Core"):
                    num_genes[curr_cluster]["core"] = int(
                        l.strip().split()[-1])
                    continue
                if l.startswith("Soft"):
                    num_genes[curr_cluster]["soft_core"] = int(
                        l.strip().split()[-1])
                    continue
                if l.startswith("Shell"):
                    num_genes[curr_cluster]["inter"] = int(
                        l.strip().split()[-1])
                    continue
                if l.startswith("Cloud"):
                    num_genes[curr_cluster]["rare"] = int(
                        l.strip().split()[-1])
    return num_genes


def parse_blast_input(input_file, gene_class, args):
...
def summarise_blast_outputs(dirs, dbs, num_genes, args):
    ...
    out.write("Cluster, Gene_class, DB, Count, Proportion, Genes\n")
    for d in dirs:
        curr_cluster = d.split("/")[-1].split("_")[0]
        for gene_class in ["core", "soft_core", "inter", "rare"]:
            for db in dbs:
                input_file = os.path.join(
                    d, gene_class + "_" + db + "_blast_result.tab")
                hits = parse_blast_input(input_file, gene_class, args)
                cnt = float(len(hits))
                genes = ";".join(hits)
                out.write(",".join(map(str, [
                          curr_cluster, gene_class, db, cnt, cnt / num_genes[curr_cluster][gene_class], genes])) + "\n")
    out.close()
    return
...
def get_options():
    parser = argparse.ArgumentParser(
        description='Count how many of the DB genes are found in each gene class')
    # input options
    ...
    parser.add_argument('--input_dir', required=False, type=str,
                        default="/lustre/scratch118/infgen/team216/gh11/e_coli_collections/poppunk/dists_analysis/roary_outputs/", help='path to input directory [%(default)s]')
